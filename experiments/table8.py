"""Table 8: Binary Join Selectivity - Best results (fixed hyperparameters from paper).

Reproduces the 4 rows of Table 8:
  M2_dnn + AE_C2 + dH5 (1024,512,512,256,256)
...
def run(data_dir, output_dir, **kwargs):
    """Run Table 8 experiment with fixed hyperparameters from paper."""
    print("\n" + "=" * 60)
    print("TABLE 8: Binary Join Selectivity - Best Results")
    print("=" * 60)

    # Fixed experiments from paper Table 8
    experiments = [
        # (m2_arch, m2_type, ae_name, m2_cfg_name)
        ("M2_DNN", "dnn", "AE_C2", "dH5"),
